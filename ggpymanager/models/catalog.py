...
class Catalog:
    ...
    def _create_status_log(self, data: dict) -> xr.Dataset:
        ...
        # Add metadata to coordinates and convert float64 to float32
        for var in ds.variables:
            if ds[var].dtype == np.float64:
                ds[var] = ds[var].astype(np.float32)
            if var in CONFIG.NETCDF_METADATA:
                ds[var].attrs = CONFIG.NETCDF_METADATA[str(var)]
            else:
                raise KeyError(
                    f"Metadata for variable '{var}' not found in NETCDF_METADATA."
                )
